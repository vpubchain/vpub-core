...
Vpub Core 0.15.1 contains the following changes to the RPC interface and `particl-cli` for multi-wallet:

* When running Vpub Core with a single wallet, there are **no** changes to the RPC interface or `particl-cli`. All RPC calls and `particl-cli` commands continue to work as before.
...
Always backup your wallet.dat file before upgrading. If you encounter any issues, please let us know by reporting them at https://github.com/particl/vpub-core/issues
